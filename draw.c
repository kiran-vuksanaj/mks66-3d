...
struct matrix * generate_sphere(double cx, double cy, double cz,
                                double r, int step ) {
  struct matrix * out = new_matrix(4,step*step);
  double d_phi = 2 * M_PI / step;
  double d_theta = M_PI / step;
  double phi,theta;
  double x,y,z;
  for( phi = 0; phi < 2 * M_PI; phi += d_phi ){
    for( theta = 0; theta < M_PI; theta += d_theta ) {
      x = r*cos(theta) + cx;
...
void add_torus( struct matrix * edges, 
  ...
  int col;
  for(col = 0; col < torus->lastcol; col++){
    add_edge(edges,
	     torus->m[0][col],
	     torus->m[1][col],
	     torus->m[2][col],
...
struct matrix * generate_torus( double cx, double cy, double cz,
                                double r1, double r2, int step ) {
  struct matrix * out = new_matrix(4,step*step);
  double d_step = 2 * M_PI / step;
  double phi,theta,x,y,z;
  for( phi = 0; phi < 2 * M_PI; phi += d_step ){
    for( theta = 0; theta < 2 *  M_PI; theta += d_step ){
